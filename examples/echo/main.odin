...
main :: proc() {
	...
	client := nbn_cli.create("echo-example", "127.0.0.1", 42042)

	if nbn_cli.start(client) < 0 {
	...
	for !exit {
		...
		for has_event {
			...
			case .Message_Received:
				reader := nbn_cli.read_message(client)
				len: u32
		...
		if connected {
			writer := nbn_cli.create_reliable_message(client, 0)
